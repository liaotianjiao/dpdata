...
symbols = ['X', 'H', 'He', 'Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne', 'Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'Ar', 'K',
...
def to_system_data(file_name):
    ...
    local_type = list(set(atom_symbols))
    local_type.sort(key=atom_symbols.index)
    atom_names = []
    atom_numbs = []
    atom_types = []
    temp = 0
    for i in range(len(local_type)):
        idx = atom_symbols.count(local_type[i])
        atom_names.append(local_type[i])
        atom_numbs.append(idx)
        for j in range(idx):
